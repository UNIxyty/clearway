"""
Afghanistan AIP PDF Scraper
Extracts operational data from Afghanistan AIP PDF
ICAO Prefix: OA
"""
...
class AfghanistanAIPScraperPDF:
    """Scraper for Afghanistan AIP PDF files"""

    def __init__(self):
        self.aip_dir = Path(__file__).parent.parent / "AIP's"
        self.pdf_path = self.aip_dir / "afghanistan_aip.pdf"
        self.airport_codes = self._extract_airport_codes()

    def _extract_text(self) -> str:
        """Extract all text from Afghanistan AIP PDF"""
        try:
            ...
            print(f"Error reading Afghanistan AIP: {e}")
            return ""

    def _extract_airport_codes(self) -> List[str]:
        """Extract all airport codes from Afghanistan AIP"""
        text = self._extract_text()
        if not text:
            return []

        # Afghanistan uses OA prefix
        codes = set()
        pattern = r'\b(OA[A-Z]{2})\b'
    ...
    def get_airport_info(self, airport_code: str) -> Dict:
        """
        Get operational information for an Afghanistan airport
        Returns the standard 9-field structure
        """
        airport_code = airport_code.upper()

        if airport_code not in self.airport_codes:
            return {"error": f"Airport code {airport_code} not found in Afghanistan AIP"}
    ...
    def _extract_contacts(self, text: str) -> List[Dict]:
        ...
        for i, line in enumerate(lines):
            if re.match(r'(Chief|Head|Officer|Manager|Director|Contact)', line, re.IGNORECASE):
            ...
            phone_match = re.search(r'(?:Phone|Tel):\s*([+\d\s\(\)]+)', line)
...
if __name__ == "__main__":
    scraper = AfghanistanAIPScraperPDF()
    print(f"Found {len(scraper.airport_codes)} Afghanistan airports: {', '.join(scraper.airport_codes)}")
