...
class KazakhstanAIPScraperPlaywright:
	def __init__(self, base_url: str = "https://www.ans.kz/en/ais/eaip"):
	...
	def _setup_browser(self):
		...
		try:
			...
			logger.info("Playwright browser initialized for Kazakhstan AIP (headless)")
	...
	def _navigate_to_current_aip(self):
		...
		try:
			# Find link containing "eAIP Kazakhstan effective" or AIRAC pattern
			date_link = self.page.locator("a[href*='eAIP'][href*='AIRAC'], a:has-text('eAIP Kazakhstan effective')").first
			if date_link.count() > 0:
				href = date_link.get_attribute('href')
				link_text = date_link.text_content()
				logger.info(f"Found current AIP link: {link_text}, href: {href}")
			...
			all_links = self.page.locator("a[href*='AIRAC'][href*='index-en-GB']")
	...
	def _open_airport(self, airport_code: str):
		"""Navigate to specific airport page."""
		...
		# Kazakhstan AIP patterns: UAAA-AD-2.XXXX-en-GB.html or AD-2.XXXX-en-GB.html
		patterns = [
			f"{base_parsed.scheme}://{base_parsed.netloc}{base_path}/eAIP/UAAA-AD-2.{airport_code}-en-GB.html",
		...
		for pattern_url in patterns:
			try:
				...
				if airport_code in text.upper() or 'AD 2.' in text:
					logger.info(f"Successfully navigated to {airport_code}")
					return
			except Exception as e:
		...
		try:
			...
			logger.warning(f"Navigation via link click failed: {e}")
	...
	def _parse_contacts(self, text: str) -> List[Dict]:
		...
		if start_idx != -1:
			...
			# Phone numbers (Kazakhstan: +7)
			phone_regex = r'(\+7[0-9\s\-]+|\+?[0-9\s\-]{8,})'
...
def main():
	scraper = KazakhstanAIPScraperPlaywright()
	try:
		# Example: Almaty Airport (UAAA)
		result = scraper.get_airport_info("UAAA")
		import json
		print("\n=== RESULTS ===")
