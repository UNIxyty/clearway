...
def main():
	s = {class_name}()
	print(s.get_airport_info("{example_code}"))
...
def class_name(country: str) -> str:
    return country.replace(" ", "").replace("-", "").replace("'", "") + "AIPScraperPlaywright"


def pick_example_code(country: str) -> str:
    # Heuristic examples by country prefix (fallback to generic)
    hints = {
        'Finland': 'EFHK', 'Estonia': 'EETN', 'Latvia': 'EVRA', 'Sweden': 'ESSA', 'Norway': 'ENGM',
        'Poland': 'EPWA', 'Netherlands': 'EHAM', 'Belgium': 'EBBR', 'Hungary': 'LHBP', 'Portugal': 'LPPT',
        'United Kingdom': 'EGLL', 'France': 'LFPG', 'Germany': 'EDDF', 'Georgia': 'UGTB', 'Kazakhstan': 'UACC',
    }
    return hints.get(country, 'XXXX')


def generate(json_path: Path, out_dir: Path):
    data = json.loads(json_path.read_text(encoding='utf-8'))
    out_dir.mkdir(parents=True, exist_ok=True)
    created = []
    for entry in data:
        base = (entry.get('baseUrl') or '').strip()
        index_url = (entry.get('indexUrl') or '').strip()
        country = (entry.get('country') or '').strip()
        if not country or not (base or index_url):
            continue
        fname = f"{slugify(country)}_aip_scraper_playwright.py"
        cls = class_name(country)
        code = pick_example_code(country)
        # Avoid format() on TEMPLATE f-like braces by simple replacement
        src = TEMPLATE.replace('{class_name}', cls).replace('{index_url}', index_url).replace('{base_url}', base).replace('{example_code}', code)
        (out_dir / fname).write_text(src, encoding='utf-8')
...
def main():
    ...
    report = root / 'aip_web_table_report.json'
    out = root / 'scrapers'
    created = generate(report, out)
    print("Created " + str(len(created)) + " scraper(s):")
    for f in created[:20]:
        print(" - " + f)
