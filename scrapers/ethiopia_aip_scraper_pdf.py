"""
Ethiopia AIP PDF Scraper
Extracts operational data from Ethiopia AIP PDF
ICAO Prefix: HA
"""
...
class EthiopiaAIPScraperPDF:
    """Scraper for Ethiopia AIP PDF files"""

    def __init__(self):
        self.aip_dir = Path(__file__).parent.parent / "AIP's"
        self.pdf_path = self.aip_dir / "ethiopia_aip.pdf"
        self.airport_codes = self._extract_airport_codes()

    def _extract_text(self) -> str:
        """Extract all text from Ethiopia AIP PDF"""
        try:
            ...
            print(f"Error reading Ethiopia AIP: {e}")
            return ""

    def _extract_airport_codes(self) -> List[str]:
        """Extract all airport codes from Ethiopia AIP"""
        text = self._extract_text()
        if not text:
            return []

        # Ethiopia uses HA prefix
        codes = set()
        pattern = r'\b(HA[A-Z]{2})\b'
    ...
    def get_airport_info(self, airport_code: str) -> Dict:
        """
        Get operational information for an Ethiopia airport
        Returns the standard 9-field structure
        """
        airport_code = airport_code.upper()

        if airport_code not in self.airport_codes:
            return {"error": f"Airport code {airport_code} not found in Ethiopia AIP"}
    ...
    def _extract_contacts(self, text: str) -> List[Dict]:
        ...
        for i, line in enumerate(lines):
            if re.match(r'(Chief|Head|Officer|Manager|Director|Airport)', line, re.IGNORECASE):
...
if __name__ == "__main__":
    scraper = EthiopiaAIPScraperPDF()
    print(f"Found {len(scraper.airport_codes)} Ethiopia airports: {', '.join(scraper.airport_codes)}")
